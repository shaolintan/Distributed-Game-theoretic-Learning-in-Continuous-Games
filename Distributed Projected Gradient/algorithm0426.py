...
def baseline_2(N,x,T,k):
    ...
    for iter_ in range(T):
        ...
        x_t = x_list[-1]
        for i in range(N):
            x_tplus.append(func(x_t[i]-step_fun(iter_+1,k)*g_i(x_t,i)) )
        x_list.append(x_tplus)
    return x_list
...
def iter1_identical(Z,N,W,iterations,k):
    z_list = [Z.copy()]
    for iter_ in range(iterations):
        z_tplus = []
        z_t = z_list[-1]
        for i in range(N):
            tmp = sum([W[i,j]*z_t[j,:] for j in range(N)])-step_fun(iter_+1,k)*gi_bar(z_t[i,:],i)
            tmp[i] = func(tmp[i])
            z_tplus.append( tmp )
        z_tplus = np.vstack((z_tplus))
        z_list.append(z_tplus)
...
def iter2_identical(Z,N,thet,W,iterations,k):
    z_list = [Z.copy()]
    for iter_ in range(iterations):
        z_tplus = []
        z_t = z_list[-1]
        for i in range(N):
            tmp = z_t[i]-step_fun(iter_+1,k)*(z_t[i]-sum([W[i,j]*z_t[j,:] for j in range(N)])+
                                    thet[i]*gi_bar(z_t[i],i))
            tmp[i] = func(tmp[i])
            z_tplus.append( tmp )
        z_tplus = np.vstack((z_tplus))
...
x_list_2 = baseline_2(N,x,iterations,k=0)
x_list=np.array(x_list)
e_03=Nash_error_2(x_list[-1],x_list_2)
plt.loglog(range(1,iterations+2),e_03/e_03[0],'r-')
...
#Lambda=[0.01 for i in range(N)]
##
#
######  1-1  ####
#z_list_0 = iter1_identical(Z,N,W,iterations,k=0)
#z_list_0=np.array(z_list_0)
#e_04=Nash_error(x_list[-1],z_list_0)
#plt.loglog(range(1,iterations+2),e_04/e_04[0],'b--')
##
...
N=20
g=nx.cycle_graph(20)
W=graph_to_mixing_matrix(g,2)
W2=graph_to_mixing_matrix(g,4)
...
######  2-1  ####
thet=[0.2]*20
#z_list_5 = iter2_identical(Z,N,thet,W,iterations,k=0)
#z_list_5=np.array(z_list_5)
#e_05=Nash_error(x_list[-1],z_list_5)
#plt.loglog(range(1,iterations+2),e_05/e_05[0],'k--')
#
z1_list_5 = iter2_1(z_0,N,thet,lambd,W,iterations)
